...
"""This module implements algorithms that has been desciped by Joseph Schillinger."""
...
def __setrange(chroma=1, stop=10):
    return set(range(0, stop, chroma))


def __superunion(*sets):
    r = set([])
    for s in sets:
        r = r | s
    return r
...
def synchronize(*generators, convert___set2ls=True):
    ...
    gen = (__setrange(generator, cp) for generator in generators)
    union = __superunion(*gen)
    if convert___set2ls is True:
        return __set2ls(union, cp)
    else:
        return union
...
def synchronize_complementary(*generators):
    ...
    gen = (__setrange(com_generator, cp) for com_generator in com_fac)
    return __set2ls(__superunion(*gen), cp)


def fractionize(*generators):
    def mk_fractionized_gen(generator, major, stop):
        new_gen = __setrange(generator, generator * major)
        gen = (
            set([el + (major * counter) for el in new_gen])
            for counter in range(major - generator + 1)
        )
        return __superunion(*gen)

    major = max(generators)
    stop = pow(major, 2)  # since complementary_factor of "a" is "a" -> a * a
    gen = (
        mk_fractionized_gen(generator, major, stop)
        for generator in filter(lambda x: x < major, generators)
    )
    return __set2ls(__superunion(*gen, __setrange(major, stop)), stop)


def mk_complementary_factors(*generators):
    """makes complementary_factors of input generators"""
    cp = functools.reduce(lambda x, y: x * y, generators)  # common_product
...
def permute_general(iterable, lv, function):
    ...
    for counter in range(lv):
        ...
        if counter > 0:
            iterable = (
                functools.reduce(lambda x, y: x + y, element) for element in iterable
            )
    return iterable
...
def distributive_power(*args, power=2):
    ...
    def distribute_products(terms, term_coefficents, integers, coefficients, power):
    ...
    if power > 1:
        var = " + ".join("x{0}".format(counter) for counter in range(len(args)))
        ...
        term_coefficents = [count_coefficients(t, coefficients) for t in seperated_term]
